...
## Current Local Progress: Product Detail Title And Purchase Box Quality

- 商品詳細の画像ギャラリー上に、商品名の重複タイトルを出さない。
- 商品名はprimary H1として1回だけ表示する。
- パンくずの現在地はフル商品名ではなく `商品詳細` とし、SEO用JSON-LDにはフル商品名を保持する。
- 商品詳細の購入ボックスから、カート追加、今すぐ購入、カート確認へ自然に進めることを公開URLで検証する。
- Playwrightで、H1一意性、商品名の表示回数、カート追加、今すぐ購入後のcheckout内容を検証する。
...
## Current Local Progress: Cart Quantity Stepper Retail Control

- cartの商品行の数量操作を、小さな数値入力だけでなく `- / 数量 / +` の購入者向けステッパーにする。
- 数量は1〜99点に正規化し、増減後すぐに小計、点数、注文合計へ反映する。
- 削除後に商品が0件になった場合は、空状態と復帰導線を表示する。
- Playwrightで数量増加、数量減少、直接入力、削除後の合計、空状態復帰を公開URLで検証する。
...
- 問い合わせフォームの正常入力後に、注文履歴、配送条件、返品条件へ進めるリンクを出す。
- checkout配送先確認後に、カート、問い合わせ、配送条件、返品条件へ進めるリンクを出す。
- 成功したふりはしない。入力確認と次導線だけを表示する。
- Playwrightで注文検索、ログイン/登録、問い合わせ、checkout配送先確認のリンク、完了偽装文言がないことを固定する。

## Current Local Progress: Product Purchase Box Support Links

- 商品詳細の購入ボックス内に、カート確認、配送条件、返品条件、問い合わせへの購入前確認リンクをまとめる。
- 購入ボックスから配送・返品・問い合わせへすぐ進めるようにし、購入前の不安を減らす。
- 可視SEO説明や内部実装説明は追加しない。
- Playwrightで `public-product-purchase-support` のリンクと、カート追加/今すぐ購入の既存動作を公開URLで検証する。

## Current Local Progress: Product Purchase Box Link Deduplication

- 商品詳細の購入ボックス内で `カートを見る` と `カートを確認` が近接して重複している状態を解消する。
- 主操作の下には `カートを見る` を残し、購入前確認リンクは配送条件、返品条件、問い合わせに絞る。
- 購入ボックスの高さと視線移動を減らし、購入操作、条件確認、問い合わせの役割を明確にする。
- 可視SEO説明や内部実装説明は追加しない。
- Playwrightで購入前確認リンクの3導線と、`カートを見る` の既存導線を公開URLで検証する。
